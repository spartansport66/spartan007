...
const WebsiteOrderExtractor = () => {
  ...
  const extractWebsiteOrder = (text: string): ExtractedOrder | null => {
    const orderNoMatch = text.match(/Invoice Number\s*:\s*([^\n\s]+)/);
    ...
    const amountMatch = text.match(/TOTAL \(Including GST\)\s*([\d,]+\.\d{2})/);
    const amount = amountMatch ? amountMatch[1].replace(/,/g, '') : "0.00";

    const items: string[] = [];
    const productBlockMatch = text.match(/Amount\s*\n([\s\S]+?)\n\s*TOTAL/i);
    if (productBlockMatch) {
        const lines = productBlockMatch[1].split('\n').filter(line => line.trim() !== '');
        for (const line of lines) {
            const lineMatch = line.match(/^\s*\d+\s+(.+?)\s+\d{8}/);
            if (lineMatch) {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        const order = extractWebsiteOrder(pageText);
        if (order) {
      ...
      if (allExtracted.length === 0) {
        throw new Error("No valid order patterns found. Check the Debug View.");
      }
        ...
        <Card className="mb-8 border-2 border-primary/20 shadow-xl">
          <CardHeader className="bg-gray-700 text-white rounded-t-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl">Website Invoice Extractor</CardTitle>
                <CardDescription className="text-gray-200">
                  Upload bulk order invoice PDFs to extract order details.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="relative">
                  <input
                    ...
                    disabled={loading}
                  />
                  <Button disabled={loading} className="bg-gray-800 hover:bg-gray-900 text-white min-w-[200px]">
                  ...
                  <TableBody>
                    {extractedOrders.map((order, index) => (
                      <TableRow key={index} className="hover:bg-muted/30">
                        ...
                        <TableCell className="text-sm max-w-xs truncate" title={order.item}>{order.item}</TableCell>
...
export default WebsiteOrderExtractor;