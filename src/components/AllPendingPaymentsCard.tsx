...
interface PendingPaymentItem {
  ...
  order_id: string | null; // Null if against opening balance
  order_number: number | null; // Null if against opening balance
  dealer_name: string;
  dealer_phone: string;
  amount: number; // amount from payment record
...
const AllPendingPaymentsCard: React.FC<AllPendingPaymentsCardProps> = ({ onPaymentAction }) => {
  ...
  const fetchPaymentsPendingApproval = useCallback(async () => {
    ...
    try {
      ...
      // We need to fetch payments linked to orders AND payments linked directly to dealers (order_id is null)
      ...
      const formattedPayments: PendingPaymentItem[] = (paymentsPendingApproval || [])
        .map((payment: any) => {
          const isBalancePayment = payment.order_id === null;
          
          return {
            type: 'payment_pending_approval',
            id: payment.id, // Payment ID
            order_id: payment.order_id,
            order_number: isBalancePayment ? null : payment.orders?.order_number || null,
            dealer_name: payment.dealers?.name || 'N/A',
            dealer_phone: payment.dealers?.phone || '',
            amount: payment.amount,
            payment_status: payment.status, // 'pending_approval'
            payment_due_date: isBalancePayment ? null : payment.orders?.payment_due_date || null,
    ...
    try {
      const response = await fetch(APPROVE_PAYMENT_EDGE_FUNCTION_URL, {
        ...
        body: JSON.stringify({
          ...
          orderId: payment.order_id, // This is the order ID (can be null)
                    ...
                    return (
                      ...
                      >
                        <TableCell className="font-medium text-foreground">
                          {isBalancePayment ? 'BALANCE' : `#${payment.order_number}`}
                        </TableCell>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    Are you sure you want to approve the payment of ₹{payment.amount.toFixed(2)} for {isBalancePayment ? 'the dealer\'s opening balance' : `Order #${payment.order_number}`} from {payment.dealer_name}? This will mark the payment as completed and update the dealer's credit.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Are you sure you want to reject the payment of ₹{payment.amount.toFixed(2)} for {isBalancePayment ? 'the dealer\'s opening balance' : `Order #${payment.order_number}`} from {payment.dealer_name}? This will delete the payment record and revert the order (if applicable) to pending status.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancel</AlertDialogCancel>
