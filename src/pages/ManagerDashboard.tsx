...
import PaymentOverviewCard from '@/components/PaymentOverviewCard'; // Added
...
import ManagerTodayFollowupsCard from '@/components/ManagerTodayFollowupsCard';
import ManagerTodayVisitsCard from '@/components/ManagerTodayVisitsCard';
import ManagerTotalPendingOrdersCard from '@/components/ManagerTotalPendingOrdersCard';

const ManagerDashboard = () => {
  const navigate = useNavigate();
  const { user, loading: sessionLoading, isManager, userType, session } = useSession();
  ...
  const [isPaymentsReportOpen, setIsPaymentsReportOpen] = useState(false); // Added
  ...
  // Added state for Payments Report
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch total sales value (using the RPC if available, or client-side aggregation)
      const { data: salesRpcData, error: salesRpcError } = await supabase
        .rpc('get_net_sales_value')
        .single();
        
      if (salesRpcError) {
        console.warn('ManagerDashboard: RPC get_net_sales_value failed. Falling back to client-side calculation.', salesRpcError);
        // Fallback to client-side calculation if RPC fails or is not deployed
        const { data: ordersData, error: ordersDataError } = await supabase
          .from('orders')
          .select('total_amount, discount_amount');

        if (ordersDataError) {
          console.error('Error fetching orders for sales calculation:', ordersDataError);
          setTotalSalesValue(0);
        } else {
          const netTotalSales = (ordersData || []).reduce(
            (sum, order) => sum + (order.total_amount - (order.discount_amount || 0)), 
            0
          );
          setTotalSalesValue(netTotalSales);
        }
      } else {
  ...
  }, [sessionLoading, user, userType, isManager, fetchDashboardData, fetchCompanyInfo, navigate]);
  ...
  // Added handler for Payments Report
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6"><ManagerTodayFollowupsCard key={`manager-followups-${refreshKey}`} onViewReport={() => setIsSalesPersonTodayFollowupsReportOpen(true)} /><ManagerTodayVisitsCard key={`manager-visits-${refreshKey}`} onViewReport={() => setIsSalesPersonVisitReportOpen(true)} /><ManagerTotalPendingOrdersCard key={`manager-pending-orders-${refreshKey}`} onViewReport={() => setIsOrdersAwaitingDispatchReportOpen(true)} /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <PaymentOverviewCard key={`payment-overview-${refreshKey}`} onViewReport={handleViewPaymentsReport} /> {/* Added Payment Overview Card */}
