...
const SalesPersonPerformanceOverviewCard: React.FC<SalesPersonPerformanceOverviewCardProps> = ({ onViewDetails }) => {
  ...
  const [combinedPendingTarget, setCombinedPendingTarget] = useState<number>(0); // New state for pending target
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground">Total Combined Pending Target:</span>
              </div>
              <span className={`text-lg font-bold ${combinedPendingTarget > 0 ? 'text-red-600' : 'text-green-600'}`}>
                ₹{combinedPendingTarget.toFixed(2)}
              </span>
            </div>
            <Separator />
            <div className="flex items-center justify-between">
